...
let createLanguageServiceHostClass = typescript => {
  return class {
    constructor(parsedConfig, transformers) {
      _defineProperty$6(this, "parsedConfig", void 0), _defineProperty$6(this, "transformers", void 0), 
      _defineProperty$6(this, "cwd", void 0), _defineProperty$6(this, "snapshots", void 0), 
      _defineProperty$6(this, "versions", void 0), _defineProperty$6(this, "fileNames", void 0), 
      _defineProperty$6(this, "service", void 0), this.parsedConfig = parsedConfig, this.transformers = transformers, 
...
async function nonMemoizedGetService(typescript, configFileName) {
...
let getService = weakMemoize(typescript => memoize(async configFileName => nonMemoizedGetService(typescript, configFileName)));
...
async function createDeclarationCreator(dirname, pkgName) {
  ...
  let {service: service, options: options, program: program} = await (normalizePath(configFileName) === normalizePath(path__default.join(dirname, "tsconfig.json")) ? nonMemoizedGetService(typescript, configFileName) : getService(typescript)(configFileName)), moduleResolutionCache = typescript.createModuleResolutionCache(dirname, x => x, options), normalizedDirname = normalizePath(dirname);
  return {
    getDeps: entrypoints => {
      ...
      return function searchDeps(deps) {
        for (let dep of deps) {
          ...
          for (let {text: text} of sourceFile.imports) {
            ...
            resolvedModule && (allDeps.has(resolvedModule.resolvedFileName) || resolvedModule.isExternalLibraryImport || !resolvedModule.resolvedFileName.includes(normalizedDirname) || (internalDeps.add(resolvedModule.resolvedFileName), 
    ...
    getDeclarationFiles: async filename => {
      ...
      return service.getEmitOutput(filename, !0, !0).outputFiles.reduce((emitted, {name: name, text: text}) => (name.endsWith(".d.ts") && (emitted.types = {
...
function typescriptDeclarations(pkg) {
  return pkg.entrypoints.some(({source: source}) => isTsPath(source)) ? {
    ...
    async generateBundle(opts, bundle) {
      let creator = await createDeclarationCreator(pkg.directory, pkg.name), srcFilenameToDtsFilenameMap = new Map, deps = creator.getDeps(pkg.entrypoints.map(x => x.source));
      await Promise.all([ ...deps ].map(async dep => {
        let {types: types, map: map} = await creator.getDeclarationFiles(dep);
        if (srcFilenameToDtsFilenameMap.set(normalizePath(dep), types.name), this.emitFile({
          type: "asset",
          fileName: path__default.relative(opts.dir, types.name),
          source: types.content
        }), map) {
          const sourceRoot = normalizePath(path__default.dirname(path__default.relative(path__default.dirname(map.name), dep))), source = overwriteDeclarationMapSourceRoot(map.content, sourceRoot);
          this.emitFile({
            type: "asset",
...
let worker, shouldUseWorker = "true" !== process.env.DISABLE_PRECONSTRUCT_WORKER && !isCI, unsafeRequire$2 = require;
