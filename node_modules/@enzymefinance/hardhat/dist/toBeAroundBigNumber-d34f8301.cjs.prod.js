...
function toCostLessThan(received, expected, tolerance) {
  ...
  var toleranceBn = ethers.BigNumber.from((tolerance !== null && tolerance !== void 0 ? tolerance : defaultTolerance$1) * 100);
  ...
  return ensureBigNumbers(received.gasUsed, expected, this.isNot, function (received, expected) {
    ...
    var message = pass ? function () {
      return jestMatcherUtils.matcherHint('.toCostLessThan', "".concat(received), "".concat(expected, " [").concat(toleranceBn, "% tolerance]"), _this);
    } : function () {
      return jestMatcherUtils.matcherHint('.toCostLessThan', "".concat(received), "".concat(expected, " [").concat(toleranceBn, "% tolerance]"), _this);
    };
    return {
...
function toBeAroundBigNumber(received, expected, tolerance) {
  ...
  return ensureBigNumbers(received, expected, this.isNot, function (received, expected) {
    var toleranceBn = ethers.BigNumber.from((tolerance !== null && tolerance !== void 0 ? tolerance : defaultTolerance) * 100);

    if (!(toleranceBn.lt(100) && toleranceBn.gt(0))) {
      throw new Error('Tolerance must be between 0% and 100%');
    }

    var min = expected.mul(ethers.BigNumber.from(100).sub(toleranceBn)).div(ethers.BigNumber.from(100));
    var max = expected.mul(ethers.BigNumber.from(100).add(toleranceBn)).div(ethers.BigNumber.from(100));
    var pass = received.gte(min) && received.lte(max);

    var message = function message() {
      return jestMatcherUtils.matcherHint('.toBeAroundBigNumber', "".concat(received), "".concat(expected), _this);
    };
...
exports.ensureBigNumbers = ensureBigNumbers;
exports.forceFail = forceFail;
exports.isTransactionReceipt = isTransactionReceipt;
exports.setBeAroundBigNumberAssertionTolerance = setBeAroundBigNumberAssertionTolerance;
exports.setGasCostAssertionTolerance = setGasCostAssertionTolerance;
exports.setIgnoreGasMatchers = setIgnoreGasMatchers;
exports.toBeAroundBigNumber = toBeAroundBigNumber;
exports.toCostLessThan = toCostLessThan;
