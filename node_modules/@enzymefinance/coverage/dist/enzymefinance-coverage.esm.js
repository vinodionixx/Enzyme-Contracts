...
function getHash(id) {
  return utils.id("".concat(id, ":").concat(v4()));
}

function getMethodIdentifier(id) {
  return "c_".concat(utils.id(id).slice(0, 10));
}

function getHashMethodDefinition(id) {
  var hash = utils.id(id).slice(0, 10);
...
function parse(source) {
  var ast = SolidityParser.parse(source, {
    loc: true,
    range: true
  });
  var state = {
...
function mergeCoverageReports(recording, metadata) {
  ...
  var coverage = createCoverageMap();
  Object.keys(metadata.targets).forEach(function (contract) {
    var file = createFileCoverage({
      b: {},
...
export { createCoverageCollector, instrumentSource, instrumentSources, mergeCoverageReports };
