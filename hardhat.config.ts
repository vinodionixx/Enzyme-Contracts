...
function node(networkName: string) {
  ...
  const fallback = 'https://kovan.infura.io/v3/bd96e35c378044778b218e3c28bf61e5';
...
function accounts(networkName: string) {
  const uppercase = networkName.toUpperCase();
  const accounts = process.env[`ETHEREUM_ACCOUNTS_${uppercase}`] || process.env.ETHEREUM_ACCOUNTS_KOVAN || '';
...
const mnemonic = 'fluid alcohol tent dinner turn vote income tongue wink thrive kitten trouble';
...
const config: HardhatUserConfig = {
  ...
  networks: {
    hardhat: {
      ...
      gas: 21000000,
      gasPrice: 1000000000000, // TODO: Consider removing this again.
